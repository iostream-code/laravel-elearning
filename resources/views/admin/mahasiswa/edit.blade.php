...
@section('content')
    <form class="max-w-full" action="{{ route('mahasiswa_update', $mahasiswa) }}" method="POST">
        @csrf
        @method('PATCH')
                    ...
                    <div class="flex flex-col items-center pb-10">
                        @if ($mahasiswa->user->photo != null)
                            <img class="w-24 h-24 mb-3 rounded-full shadow-lg"
                                src="{{ url('storage/' . $mahasiswa->user->photo) }}" alt="Profile Picture">
                        @else
                            <img class="w-24 h-24 mb-3 rounded-full shadow-lg"
                        ...
                        <span
                            class="text-sm text-gray-500 dark:text-gray-400 capitalize">{{ $mahasiswa->user->status }}</span>
                    </div>
                </div>
            </div>
            <div
                ...
                <div class="grid md:grid-cols-2 md:gap-6">
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="nim" id="floating_nim"
                            ...
                            value="{{ $mahasiswa->nim }}" placeholder=" " required />
                ...
                <div class="grid md:grid-cols-2 md:gap-6">
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="date" name="tgl_lahir" id="floating_tgl_lahir"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            value="{{ $mahasiswa->tgl_lahir }}" placeholder=" " />
                ...
                <div class="mb-5">
                    <input
                        ...
                        aria-describedby="mahasiswa_avatar_help" id="mahasiswa_avatar" name="photo"
                        value="{{ $mahasiswa->user->photo }}" type="file">
                </div>
