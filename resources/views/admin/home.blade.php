...
@section('content')
    <main>
        <div class="grid grid-cols-3 gap-4 mb-4">
            ...
            <a href="{{ route('admin_index') }}" class="text-2xl text-gray-400 dark:text-gray-500">
                <div class="flex items-center justify-center h-24 rounded bg-gray-50 dark:bg-gray-800">
                    <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <tr
                            ...
                            @if ($user->status == 'dosen')
                                ...
                                <td class="flex gap-2 px-6 py-4">
                                    <a href="{{ route('dosen_edit', $user->dosen) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                    <form action="{{ route('dosen_delete', $user->dosen) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                            ...
                            @else
                                ...
                                <td class="flex gap-2 px-6 py-4">
                                    <a href="{{ route('mahasiswa_edit', $user->mahasiswa) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                    <form action="{{ route('mahasiswa_delete', $user->mahasiswa) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="font-medium text-red-600 dark:text-red-500 hover:underline">Delete</button>
